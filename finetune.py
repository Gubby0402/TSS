...
# Initialize the accelerator for single GPU usage only
print("\n===================== 初始化Accelerator =====================")
print(f"args.mixed_precision: {args.mixed_precision}, args.fp16: {args.fp16}")
accelerator = Accelerator(
    mixed_precision="no",  # 强制设置为"no"，不使用混合精度
    fp16=False,            # 强制设置为False，不使用FP16
    # Remove any distributed training settings
...
print(f"accelerator设备: {accelerator.device}")
if hasattr(accelerator, 'scaler'):
    print(f"accelerator.scaler: {accelerator.scaler}")
    accelerator.scaler = None
    print("设置accelerator.scaler = None")
else:
    print("accelerator没有scaler属性")
print("===================== Accelerator初始化完成 =====================\n")
...
# Set verbosity based on main process
...
if args.tokenizer_name:
    ...
    add_space_tokenizer = AutoTokenizer.from_pretrained(args.base_model_name_or_path,
...
train_pool_loader = DataLoader(train_dataset, shuffle=True, collate_fn=data_collator,
                               batch_size=args.replay_sample_per_task)
