...
# TSS: New utility functions for importance computation and soft masking
def compute_model_importance(model, task_id):
    """Compute importance for all MultitaskMaskLinear modules in the model"""
    for n, m in model.named_modules():
        if isinstance(m, MultitaskMaskLinear):
            m.compute_importance(task_id)

def apply_model_soft_mask(model, task_id):
    """Apply soft masking to all MultitaskMaskLinear modules in the model"""
    for n, m in model.named_modules():
        if isinstance(m, MultitaskMaskLinear):
            m.apply_soft_mask(task_id)

def initialize_model_with_knowledge_transfer(model, task_id):
    """Initialize new task with knowledge transfer for all MultitaskMaskLinear modules"""
    for n, m in model.named_modules():
        if isinstance(m, MultitaskMaskLinear):
            m.initialize_with_knowledge_transfer(task_id)
...
class MultitaskMaskLinear(nn.Linear):
    ...
    def compute_importance(self, task_id):
        """
        Compute importance of popup scores using gradient-based method
        Args:
            task_id: current task id
        """
        if not hasattr(self, 'importance_scores'):
            return

        # Get current popup scores for the task
        if hasattr(self, 'specific_scores') and task_id < len(self.specific_scores):
            popup_scores = self.specific_scores[task_id]
            if popup_scores.grad is not None:
                # Get device from current parameter
                device = popup_scores.device

                # Compute importance as absolute average gradient
                grad_abs = torch.abs(popup_scores.grad)
                importance = grad_abs  # Keep the same shape as popup scores

                # Update importance with EMax (element-wise maximum)
                if task_id > 0 and hasattr(self, 'accumulated_importance') and self.accumulated_importance is not None:
                    # Ensure accumulated_importance is on the correct device
                    self.accumulated_importance = self.accumulated_importance.to(device)
                    # EMax operation: element-wise maximum between current and accumulated
                    self.accumulated_importance = torch.maximum(
                        self.accumulated_importance, importance.detach()
                    )
                else:
                    # First task, initialize accumulated importance
                    self.accumulated_importance = importance.detach().clone()

                # Store importance for current task
                self.importance_scores[task_id].data.copy_ \
                    (importance.detach().to(self.importance_scores[task_id].device))

    def apply_soft_mask(self, task_id):
        """
        Apply soft masking to gradients based on accumulated importance
        Args:
            task_id: current task id
        """
        if not hasattr(self, 'accumulated_importance') or task_id == 0 or self.accumulated_importance is None:
            return  # No soft masking for first task
        ...
        if hasattr(self, 'specific_scores') and task_id < len(self.specific_scores):
            ...
            if popup_scores.grad is not None:
                ...
                # Ensure accumulated_importance is on the correct device
                accumulated_importance = self.accumulated_importance.to(device)

                # Apply soft mask: grad = (1 - importance) * grad
                soft_mask = 1.0 - accumulated_importance
                # Ensure soft_mask is in [0, 1] range
                soft_mask = torch.clamp(soft_mask, 0.0, 1.0)
                popup_scores.grad.data *= soft_mask

    def initialize_with_knowledge_transfer(self, task_id):
        """
        Initialize popup scores for new task using accumulated knowledge
        Args:
            task_id: current task id
        """
        if task_id == 0 or not hasattr(self, 'accumulated_importance'):
            return  # No initialization for first task

        if hasattr(self, 'specific_scores') and task_id < len(self.specific_scores):
            # Initialize new task's popup scores with knowledge from all previous tasks
            # Use accumulated importance to guide initialization
            with torch.no_grad():
                # Get device from current parameter
                device = self.specific_scores[task_id].device

                # Start with Kaiming initialization
                new_scores = mask_init(self)

                # Ensure new_scores is on the correct device
                if hasattr(new_scores, 'to'):
                    new_scores = new_scores.to(device)

                # If we have accumulated importance, use it to guide initialization
                if hasattr(self, 'accumulated_importance') and self.accumulated_importance is not None:
                    # Ensure accumulated_importance is on the correct device
                    importance_weight = self.accumulated_importance.to(device)

                    # Get average of all previous tasks' scores weighted by importance
                    if task_id > 0:
                        prev_knowledge = torch.zeros_like(new_scores, device=device)
                        valid_prev_tasks = 0

                        for prev_task in range(task_id):
                            if prev_task < len(self.specific_scores):
                                prev_data = self.specific_scores[prev_task].data.to(device)
                                prev_knowledge += prev_data
                                valid_prev_tasks += 1

                        if valid_prev_tasks > 0:
                            prev_knowledge /= valid_prev_tasks  # Average of previous tasks

                            # Blend: new_scores = (1-w) * new_init + w * prev_knowledge
                            # where w is the importance weight
                            blend_weight = torch.clamp(importance_weight, 0.0, 0.8)  # Cap at 0.8 to allow learning
                            new_scores = (1.0 - blend_weight) * new_scores + blend_weight * prev_knowledge

                # Update the popup scores
                self.specific_scores[task_id].data.copy_(new_scores)
    ...
    def __init__(self, *args, num_tasks=1 ,adapter_role=None, N=1, my_args=None ,**kwargs):
        ...
        # Initialize importance tracking for TSS
        self.importance_scores = nn.ParameterList([
            nn.Parameter(torch.zeros_like(mask_init(self)), requires_grad=False)
