...
class MyBertSelfOutput(BertSelfOutputAdaptersMixin, nn.Module):
...
class MyBertOutput(BertOutputAdaptersMixin, nn.Module):
    def __init__(self, config, args):
...
class MyBertLayer(nn.Module):
    def __init__(self, config, args):
        ...
        self.seq_len_dim = 1
        self.attention = MyBertAttention(config, location_key="self", args=args)
        self.is_decoder = config.is_decoder
        self.add_cross_attention = config.add_cross_attention
        if self.add_cross_attention:
            if not self.is_decoder:
                raise ValueError(f"{self} should be used as a decoder model if cross attention is added")
            self.crossattention = MyBertAttention(config, position_embedding_type="absolute", location_key="cross",
                                                  args=args)
...
class MyBertModel(BertModelAdaptersMixin, BertPreTrainedModel):
    """
...
class MyBertForSequenceClassification(ModelWithHeadsAdaptersMixin, BertPreTrainedModel):
    def __init__(self, config, taskcla, args):
        ...
        self.bert = MyBertModel(config, args=args)
        classifier_dropout = (
    ...
    def forward(
        ...
        # 初始化loss为需要梯度的张量而不是标量
        loss = torch.tensor(0.0, requires_grad=True, device=pooled_output.device)
        logits = None
        ...
        if labels is not None and task is not None:
            ...
            for t_id, t in enumerate(task):  # different task in the same batch
                if self.args.is_transfer or self.args.is_reference:
                    logit = self.readouts[self.args.eval_t][t](
                        pooled_output[t_id].unsqueeze(0))  # no stack can be performed
                else:
                    logit = self.classifiers[t](pooled_output[t_id].unsqueeze(0))  # no stack can be performed
                num_labels = self.taskcla[t][1]
                cur_loss = loss_fct(logit.view(-1, num_labels), labels[t_id].view(-1))
                losses.append(cur_loss)
                logits.append(logit)

            # 使用torch.stack和sum来避免原地操作
            if losses:
                total_loss = torch.stack(losses).sum()
                loss = total_loss / len(task)
            else:
                loss = torch.tensor(0.0, requires_grad=True, device=pooled_output.device)
            ...
            for t in range(self.args.ntasks):
                logit = self.classifiers[t](pooled_output)
                logits.append(logit)
...
class MyBertForTokenClassification(ModelWithHeadsAdaptersMixin, BertPreTrainedModel):
    _keys_to_ignore_on_load_unexpected = [r"pooler"]
    ...
    def __init__(self, config, taskcla, args):
        ...
        self.bert = MyBertModel(config, add_pooling_layer=False, args=args)
        classifier_dropout = (
            config.classifier_dropout if config.classifier_dropout is not None else config.hidden_dropout_prob
        )
        self.dropout = nn.Dropout(classifier_dropout)
    ...
    def forward(
        ...
        if labels is not None and task is not None:
            ...
            for t_id, t in enumerate(task):  # different task in the same batch
                if self.args.is_transfer or self.args.is_reference:
                    logit = self.readouts[self.args.eval_t][t](
                        sequence_output[t_id].unsqueeze(0))  # no stack can be performed
                else:
