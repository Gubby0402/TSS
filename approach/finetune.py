...
class Appr(object):
    ...
    def train(self, model, train_loader, train_dataset, dev_loaders, test_loaders, train_pool_loader, accelerator):
        ...
        # TSS: Initialize knowledge transfer for supsup baseline
        if 'supsup' in self.args.baseline and self.args.ft_task > 0:
            from networks.baselines.supsup import initialize_model_with_knowledge_transfer
            model_ori = accelerator.unwrap_model(model)
            initialize_model_with_knowledge_transfer(model_ori, self.args.ft_task)
            logger.info(f"TSS: Initialized task {self.args.ft_task} with knowledge transfer from previous tasks")
        ...
        if accelerator.is_local_main_process:
            ...
            logger.info(
                f"  Num Epochs = {self.args.num_train_epochs}, Pre-trained Model = {self.args.model_name_or_path}")
            logger.info(f"  Instantaneous batch size per device = {self.args.per_device_train_batch_size}")
            logger.info(f"  Total train batch size (w. parallel, distributed & accumulation) = {total_batch_size}")
            logger.info(
                f"  Gradient Accumulation steps = {self.args.gradient_accumulation_steps}, Lamb = {self.args.lamb}")
            logger.info(
                f"  Learning Rate = {self.args.learning_rate}, Prompt Learning Rate = {self.args.prompt_lr} Adapter Learning Rate = {self.args.adapter_lr}, Classifier Learning Rate = {self.args.classifier_lr}, Warmup Num = {self.args.num_warmup_steps}")
            logger.info(f"  Total optimization steps = {self.args.max_train_steps}, NTokens={self.args.n_tokens}")
            logger.info(
                f"  Seq ID = {self.args.idrandom}, Task id = {self.args.ft_task}, Task Name = {self.args.task_name}, Num task = {self.args.ntasks}")
            ...
            for saved_output_dir in self.args.saved_output_dir[:-2]:  # we need -2 so that we can load model
                if os.path.isdir(saved_output_dir):
                    ...
                    for item in os.listdir(saved_output_dir):
                        if (item.endswith(".bin") or item.endswith(
                                ".json")) and 'adapter' not in item and 'head' not in item:
            ...
            try:
                for epoch in range(starting_epoch, self.args.num_train_epochs):
                    ...
                    for step, batch in enumerate(train_loader):
                        self, model, outputs, weights_before = compute_loss.compute(self, model, train_loader, outputs,
                                                                                    self_fisher, mask_pre, batch, step,
                                                                                    accelerator)
                        loss = outputs.loss
                        model = compute_gradeint.compute(self, model, loss, mask_back, weights_before, epoch, batch,
                                                         step, accelerator)
                        ...
                        if step % self.args.gradient_accumulation_steps == 0 or step == len(train_loader) - 1:
                            update_model.update(self, model, optimizer, outputs, loss, writer, lr_scheduler,
                                                progress_bar, global_step, completed_steps, epoch, step, accelerator)
                    ...
                    if (
                            'SemEval' in self.args.task_name or self.args.task_name in self.args.ccd_datasets or self.args.task_name in self.args.generation or self.args.task_name in self.args.ner_datasets):  # no need to test for everyone

                        results = self.eval(model, dev_loader, metric, accelerator, eval_t=self.args.ft_task,
                                            dev_mode=True)  # use this to choose
                ...
                if (
                        'SemEval' in self.args.task_name or self.args.task_name in self.args.ccd_datasets or self.args.task_name in self.args.generation or self.args.task_name in self.args.ner_datasets):
        ...
        for eval_t in range(self.args.ft_task + 1):
            ...
            pred_file = os.path.join(self.args.output_dir.replace(self.args.ori_task_name, self.args.all_tasks[eval_t]),
                                     self.args.all_tasks[eval_t] + str(self.args.ft_task) + '_pred')
            target_file = os.path.join(self.args.output_dir, self.args.all_tasks[eval_t] + '_target')

            os.makedirs(self.args.output_dir.replace(self.args.ori_task_name, self.args.all_tasks[eval_t]),
                        exist_ok=True)
    ...
    def eval(self, model, dataloader, metric, accelerator, eval_t=None, pred_file=None, target_file=None,
             tune_model=None, infer_model=None, dev_mode=False, eval_batch=None):
        model.eval()
        ...
        for step, batch in enumerate(dataloader):
            with torch.no_grad():
                ...
                if self.args.task_name in self.args.asc_datasets or self.args.task_name in self.args.ccd_datasets:
                    ...
                    if outp is None:
                        # 创建虚拟预测，假设所有样本都预测为类别0
                        pred = torch.zeros(real_b, dtype=torch.long, device=batch["input_ids"].device)
                    elif type(outp) == list:
                        pred = []
                        for out in outp:
                            if out is None:
                                pred.append(torch.zeros(1, dtype=torch.long, device=batch["input_ids"].device))
                            else:
                    ...
                    if 'prompt' in self.args.baseline or 'l2p' in self.args.baseline:
                        inputs_embeds, attention_mask = tune_model.get_prompt_extended_input_exclude_label(
