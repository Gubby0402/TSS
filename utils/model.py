...
from networks.transformers.bart import MyBartForConditionalGeneration, MyBartForSequenceClassification, \
    MyBartForTokenClassification
from networks.transformers.roberta import MyRobertaForSequenceClassification, MyRobertaForTokenClassification, \
    MyRobertaForMaskedLM
...
from networks.baselines.l2p import MyBartForSequenceClassificationSoftL2P, MyBartForConditionalGenerationSoftL2P, \
    MyBartForTokenClassificationSoftL2P
from networks.prompt.inference import MyBartForConditionalGenerationSoftPromptInfer
from networks.prompt.tuning import MyBartForConditionalGenerationSoftPromptTunning, \
    MyRobertaForTokenClassificationSoftPromptTunning, MyRobertaForSequenceClassificationSoftPromptTunning
import utils
...
def prepare_sequence_finetune(args):
    ...
    if args.eval_only:
        output = args.base_dir + "/seq" + str(args.idrandom) + "/seed" + str(args.seed) + "/" + str(
            args.baseline) + '/' + str(data[args.ft_task]) + "_lm/"
        ...
        args.prev_output = args.base_dir + "/seq" + str(args.idrandom) + "/seed" + str(args.seed) + "/" + str(
            args.baseline) + '/' + str(data[args.ft_task - 1]) + "_lm/"
    ...
    args.dialogue_datasets = ['MWOZ_taxi', 'MWOZ_train', 'MWOZ_restaurant', 'MWOZ_hotel', 'MWOZ_attraction',
                              'sgd_services',
...
def _lookfor_model_adapter(taskcla, args, config):
    ...
    if (args.ft_task == 0 or 'one' in args.baseline or 'mtl' in args.baseline or 'comb' in args.baseline) and (
    not args.eval_only):
...
def write_result(results, eval_t, args):
    ...
    if args.ft_task == args.ntasks - 1:  # last ft task, we need a final one
        ...
        if args.task_name in args.asc_datasets or args.task_name in args.ccd_datasets:
            ...
            if 'one' in args.baseline:
                with open(final_micro_f1, 'w') as micro_f1_file, open(final_macro_f1, 'w') as macro_f1_file, open(
                        final_accuracy,
                ...
                with open(final_micro_f1, 'w') as final_micro_f1_file, open(final_macro_f1,
                                                                            'w') as final_macro_f1_file, open(
                        final_accuracy, 'w') as final_accuracy_file, open(final_loss, 'w') as final_loss_file, \
                        open(forward_micro_f1, 'w') as forward_micro_f1_file, open(forward_macro_f1,
                                                                                   'w') as forward_macro_f1_file, open(
            ...
            if 'one' in args.baseline:
                with open(final_f1, 'w') as f1_file, open(final_precision, 'w') as precision_file, open(final_recall,
                                                                                                        'w') as recall_file, open(
                        final_accuracy, 'w') as accuracy_file:
            ...
            if 'one' in args.baseline:
                with open(final_rouge1, 'w') as rouge1_file, open(final_rouge2, 'w') as rouge2_file, open(final_rougeL,
                                                                                                          'w') as rougeL_file, open(
                        final_rougeLsum, 'w') as rougeLsum_file:
                ...
                with open(final_rouge1, 'w') as final_rouge1_file, open(final_rouge2, 'w') as final_rouge2_file, open(
                        final_rougeL, 'w') as final_rougeL_file, open(final_rougeLsum, 'w') as final_rougeLsum_file, \
                        open(forward_rouge1, 'w') as forward_rouge1_file, open(forward_rouge2,
                                                                               'w') as forward_rouge2_file, open(
                    forward_rougeL, 'w') as forward_rougeL_file, open(forward_rougeLsum, 'w') as forward_rougeLsum_file:
                    for j in range(rouge1.shape[1]):
...
def deep_copy(model, accelerator, args):
    ...
    optimizer_grouped_parameters = utils.optimize.lookfor_optimize(unwrap_adaptive_model,
                                                                   args)  # everything is based on adative_model
